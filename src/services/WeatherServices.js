...
const WeatherServices = () => {
  ...
  const transformCurrentWeather = (data) => {
    return {
      ...
      pressure: Math.round(data.main.pressure * 0.75) + " mm sim. ust",
      name: data.name,
  ...
  const transformDailyWeahter = (data) => {
    ...
    const helpFunc = (value) => {
      const max =
        Math.floor(Math.max.apply(null, value)) > 0
          ? `+${Math.floor(Math.max.apply(null, value))}`
          : Math.floor(Math.max.apply(null, value));
      const min =
        Math.floor(Math.min.apply(null, value)) > 0
          ? `+${Math.floor(Math.min.apply(null, value))}`
          : Math.floor(Math.min.apply(null, value));
